...
# 61 "proxy3.c"
  ...
  while (1)
  {
    ...
    Getnameinfo((SA *) &clientaddr, clientlen, hostname, 1024,
        port, 1024, 0);
...
{
  char buf[1024], headbuf[8192];
  request_t request;
  rio_t client_rio;
  ssize_t n;
  int skeep_alive = 0, ckeep_alive = 0;
  header_t header;
  size_t headlen;
...
request_line:
  if ((n = Rio_readlineb(&client_rio, buf, 1024)) <= 0)
  {
    goto end;
  }
  ...
  if (skeep_alive)
  {
    if(verbose) { printf("-------server keepalive--------\n"); };
    goto transfer;
  }
...
transfer:
  if(verbose) { printf("-------request from client--------\n"); };
  ...
  if(verbose) { printf("-------response to client--------\n"); };

  if (parse_header(&server_rio, &header, headbuf, &headlen))
  {
    skeep_alive = header.keep_alive;
  ...
  {
    while ((n = Rio_readnb(&server_rio, buf, 1024)) > 0)
    {
  ...
  if (skeep_alive && ckeep_alive)
  {
    if(verbose) { printf("----------client keepalive-------"); };
    goto request_line;
  }
  ...
  Close(serverfd);
  Close(clientfd);
  if(verbose) { printf("------close serverfd %d\n", serverfd); };
  if(verbose) { printf("------close clientfd %d\n", clientfd); };
...
int parse_header (rio_t *rrio, header_t *header, char *headbuf, size_t *buflen)
{
  ...
  *headbuf = 0;
  ...
  do
  {
    if ((n = Rio_readlineb(rrio, buf, 1024)) <= 0 )
      return 0;
