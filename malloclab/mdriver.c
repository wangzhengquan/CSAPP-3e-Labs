...
#define IS_ALIGNED(p)  ((((unsigned int)(p)) & (ALIGNMENT - 1)) == 0)
...
static void reset_system_mm (trace_t *trace);
static void reset_mm (trace_t *trace);
...
void reset_system_mm (trace_t *trace)
{
  char *p;
  for (int i = 0; i < trace->num_ids; i++)
  {
    if ((p = trace->blocks[i]) != NULL)
      free(p);
  }
...
static trace_t *read_trace(char *tracedir, char *filename)
{
  ...
  if ((trace->ops = (traceop_t *)calloc(trace->num_ops , sizeof(traceop_t))) == NULL)
  ...
  for (i = 0;  i < trace->num_ops;  i++)
  {
    ...
    if (!mm_checkheap(verbose))
    {
      sprintf(msg, "checkheap failed, newp=%p, newsize=%lx, oldp=%p, oldsize=%lx\n",
              newp, size, trace->blocks[index], trace->block_sizes[size]);
...
static void usage(void)
{
  ...
  fprintf(stderr, "\t-v <verbose> Print per-trace performance breakdowns.\n");
}
